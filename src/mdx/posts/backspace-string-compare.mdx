...
- Time complexity: <Math math="O(n + m)" />
- Space complexity: <Math math="O(n + m)" />
...
With two pointer technique (deciding when to move the pointers and what to do on each interaction), we can bring down the space complexity to <Math math="O(1)" />.
